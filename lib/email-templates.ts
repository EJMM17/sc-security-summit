...
// =============================================================
// 4. Payment reminder (sent 48h before SPEI/OXXO expiry)
// =============================================================
export type PaymentReminderData = {
  ...
  tipoAcceso: RegistroInput["tipo_acceso"];
  montoMxn: number;
  metodoPago: "spei" | "oxxo" | "tarjeta" | "transferencia_manual";
  expiresAt?: string | null;
  paymentUrl: string;
  language: EmailLanguage;
};

export function buildPaymentReminder(data: PaymentReminderData): RenderedEmail {
  const tier = TIER_LABEL[data.tipoAcceso][data.language];
  const monto = formatMxn(data.montoMxn);
  const expiresFmt = data.expiresAt
    ? new Intl.DateTimeFormat(data.language === "en" ? "en-US" : "es-MX", {
        dateStyle: "long",
        timeStyle: "short",
  ...
  if (data.language === "en") {
    const subject = `Reminder · payment pending · Folio ${data.folio}`;
    const greeting = `Hi ${data.nombre},`;
    const intro = `Your registration for the ${EVENT.title} is reserved but payment is still pending.`;
    const expiryLine = expiresFmt
      ? `Your payment reference expires on ${expiresFmt}. After that, your seat may be released.`
      : `Please complete your payment to secure your seat.`;
    const cta = "Complete payment";
    const sign = `— ${ORGANIZERS.presented_by.join(" & ")}`;
...
<div style="font-size:15px;line-height:1.6;color:#e6e8ee;">
  <p style="margin:0 0 14px;">${greeting}</p>
  <p style="margin:0 0 22px;color:#c5cad4;">${intro}</p>
  <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="background:#0d1017;border:1px solid #f59e0b;border-radius:8px;margin-bottom:24px;">
    <tr><td style="padding:18px 20px;">
      <div style="display:inline-block;padding:4px 10px;border-radius:999px;background:#78350f;color:#fcd34d;font-size:11px;letter-spacing:0.14em;text-transform:uppercase;font-weight:700;">PENDING</div>
      <div style="margin-top:12px;font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;font-size:18px;font-weight:600;color:#ffffff;letter-spacing:0.04em;">${data.folio}</div>
    </td></tr>
    <tr><td style="padding:0 20px 18px;">
      <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%">
        <tr><td style="padding-top:10px;border-top:1px solid #1f2430;font-size:13px;color:#9aa3b2;">Tier</td>
            <td style="padding-top:10px;border-top:1px solid #1f2430;font-size:13px;color:#ffffff;text-align:right;">${tier}</td></tr>
        <tr><td style="padding-top:8px;font-size:13px;color:#9aa3b2;">Amount</td>
            <td style="padding-top:8px;font-size:13px;color:#ffffff;text-align:right;font-weight:600;">${monto}</td></tr>
      </table>
    </td></tr>
  </table>
  <p style="margin:0 0 22px;color:#c5cad4;font-size:14px;">${expiryLine}</p>
  <p style="margin:0 0 24px;"><a href="${data.paymentUrl}" style="display:inline-block;background:#2563eb;color:#ffffff;text-decoration:none;padding:12px 22px;border-radius:8px;font-weight:600;font-size:14px;">${cta} →</a></p>
  <p style="margin:0;color:#9aa3b2;font-size:13px;">${sign}</p>
</div>`;

    return {
      subject,
      html: htmlShell({ previewText: `Pending: ${data.folio} · ${monto}`, bodyHtml }),
      text,
    };
  }

  const subject = `Recordatorio · pago pendiente · Folio ${data.folio}`;
  const greeting = `Hola ${data.nombre},`;
  const intro = `Tu registro para el ${EVENT.title} está reservado pero el pago sigue pendiente.`;
  const expiryLine = expiresFmt
    ? `Tu referencia de pago vence el ${expiresFmt}. Después de esa fecha tu lugar podría liberarse.`
    : `Por favor completa tu pago para asegurar tu lugar.`;
  const cta = "Completar pago";
  const sign = `— ${ORGANIZERS.presented_by.join(" & ")}`;
...
<div style="font-size:15px;line-height:1.6;color:#e6e8ee;">
  ...
  <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="background:#0d1017;border:1px solid #f59e0b;border-radius:8px;margin-bottom:24px;">
    <tr><td style="padding:18px 20px;">
      <div style="display:inline-block;padding:4px 10px;border-radius:999px;background:#78350f;color:#fcd34d;font-size:11px;letter-spacing:0.14em;text-transform:uppercase;font-weight:700;">PENDIENTE</div>
      <div style="margin-top:12px;font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;font-size:18px;font-weight:600;color:#ffffff;letter-spacing:0.04em;">${data.folio}</div>
    </td></tr>
    <tr><td style="padding:0 20px 18px;">
      <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%">
        <tr><td style="padding-top:10px;border-top:1px solid #1f2430;font-size:13px;color:#9aa3b2;">Tipo</td>
            <td style="padding-top:10px;border-top:1px solid #1f2430;font-size:13px;color:#ffffff;text-align:right;">${tier}</td></tr>
        <tr><td style="padding-top:8px;font-size:13px;color:#9aa3b2;">Monto</td>
            <td style="padding-top:8px;font-size:13px;color:#ffffff;text-align:right;font-weight:600;">${monto}</td></tr>
      </table>
    </td></tr>
  </table>
  ...
  return {
    subject,
    html: htmlShell({ previewText: `Pendiente: ${data.folio} · ${monto}`, bodyHtml }),
    text,
  };
}

// =============================================================
// 5. Waitlist notification (sent when capacity is exhausted)
// =============================================================
...
export function buildWaitlistNotification(data: WaitlistEmailData): RenderedEmail {
  if (data.language === "en") {
    const subject = `Waitlist confirmed · ${EVENT.title}`;
    const greeting = `Hi ${data.nombre},`;
    const intro = `All seats for the ${EVENT.title} are sold out — but you're now on the waitlist.`;
    const body = `If a seat opens up, we'll contact you in the order requests came in. We'll also notify you about our 2027 edition first.`;
    const sign = `— ${ORGANIZERS.presented_by.join(" & ")}`;
    const text = [greeting, "", intro, "", body, "", sign].join("\n");
    const bodyHtml = `
<div style="font-size:15px;line-height:1.6;color:#e6e8ee;">
  <p style="margin:0 0 14px;">${greeting}</p>
  <p style="margin:0 0 16px;color:#c5cad4;">${intro}</p>
  <p style="margin:0 0 24px;color:#c5cad4;">${body}</p>
  <p style="margin:0;color:#9aa3b2;font-size:13px;">${sign}</p>
</div>`;
    return {
      subject,
      html: htmlShell({ previewText: "You're on the waitlist", bodyHtml }),
      text,
    };
  }

  const subject = `Lista de espera confirmada · ${EVENT.title}`;
  const greeting = `Hola ${data.nombre},`;
  const intro = `Los cupos para el ${EVENT.title} están agotados — quedaste en lista de espera.`;
  const body = `Si se libera un lugar, te contactaremos en orden de llegada. También te avisaremos primero sobre la edición 2027.`;
  const sign = `— ${ORGANIZERS.presented_by.join(" & ")}`;
  const text = [greeting, "", intro, "", body, "", sign].join("\n");
  const bodyHtml = `
<div style="font-size:15px;line-height:1.6;color:#e6e8ee;">
  ...
  return {
    subject,
    html: htmlShell({ previewText: "Lista de espera confirmada", bodyHtml }),
    text,
  };
}
